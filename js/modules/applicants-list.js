      ...
      <article class="card" style="padding: 1.5rem; ${
        ...
        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem; flex-wrap: wrap; gap: 1rem;">
          <div style="flex: 1; min-width: 200px;">
            <h3 style="margin: 0 0 0.5rem 0; color: #263238; font-size: 1.25rem;">${
							job.title
						}</h3>
          ...
          <div style="display: flex; align-items: center; gap: 1rem;">
            ...
            <select class="form-input status-select-${
				...
				app.status
			}" style="padding: 0.5rem; border: 1px solid #E0E0E0; border-radius: 8px; font-size: 0.9rem;">
            ...
            <button class="btn btn-outline" onclick="updateApplicantStatus('${
							app.applicantId
						}', '${app.companyId}', document.querySelector('.status-select-${
				app.applicantId
			}').value)" style="padding: 0.5rem 1rem; white-space: nowrap;">Update</button>
          </div>
        </div>
        ...
        <div style="padding: 1rem; background: #F5F5F5; border-radius: 8px; margin-bottom: 1rem;">
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
            ...
            <div>
              ...
              <p style="margin: 0; color: #263238; font-family: monospace; font-size: 0.875rem;">${
								app.applicantId
							}</p>
            </div>
            <div>
        ...
        <div style="display: flex; gap: 0.5rem;">
          <button class="btn btn-primary" onclick="viewApplicantProfile('${
						app.studentId
					}')">View Profile</button>
          <button class="btn btn-outline" onclick="contactApplicant('${
						app.applicantId
					}')">Send Message</button>
        </div>
      </article>
...
function loadApplicants() {
	try {
		...
		allApplicantsData = allApplicantsData.sort(
		...
		document.getElementById('applicants-list').innerHTML =
			'<p style="text-align: center; color: #666;">Unable to load applicants. Please try again later.</p>';
	}
}
