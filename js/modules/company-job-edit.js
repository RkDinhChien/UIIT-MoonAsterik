...
function openEditJobModal(jobId) {
  ...
  document.getElementById('editJobSalaryIncrease').value = job.salaryIncrease || 0;
  document.getElementById('editJobSalary').value = job.salary || '';
  document.getElementById('editJobBonus').value = job.bonus === 'N/A' ? '' : job.bonus || '';
  ...
  if (job.technicalRequirements && job.technicalRequirements.length > 0) {
    ...
    editTechList.innerHTML = '<p id="edit-no-tech-requirements" style="width: 100%; text-align: center; color: #999; font-style: italic; padding: 1rem 0; margin: 0;">No technical requirements added yet.</p>';
  }
...
function checkEditNoTechRequirements() {
  ...
  if (techList.children.length === 0) {
    techList.innerHTML = '<p id="edit-no-tech-requirements" style="width: 100%; text-align: center; color: #999; font-style: italic; padding: 1rem 0; margin: 0;">No technical requirements added yet.</p>';
  }
}

// Helper function to get technical requirements from edit badges
function getEditTechRequirements() {
  const techBadges = document.querySelectorAll('#edit-tech-requirements-list .tech-badge');
...
function submitJobUpdate() {
  ...
  const updatedJobData = {
    ...
    salaryIncrease: parseFloat(document.getElementById('editJobSalaryIncrease').value) || 0,
