...
function loadCompanyProfile() {
	...
	const logoAvatar = document.querySelector('.profile-avatar-large');
	if (logoAvatar && profile.logo) {
		logoAvatar.innerHTML = `<img src="${profile.logo}" alt="Company Logo" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.parentElement.innerHTML='<svg style=\\"width: 60px; height: 60px; color: #999\\" viewBox=\\"0 0 24 24\\" fill=\\"none\\" stroke=\\"currentColor\\"><rect x=\\"2\\" y=\\"7\\" width=\\"20\\" height=\\"14\\" rx=\\"2\\" ry=\\"2\\"></rect><path d=\\"M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16\\"></path></svg>';" />`;
	}
	...
	if (profile.offices && profile.offices.length > 0) {
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem; margin-bottom: 1rem;">
          <div class="form-group">
            ...
            <input type="text" class="form-input office-name" value="${
							office.name || ''
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-input office-city" value="${
							office.city || ''
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-input office-country" value="${
							office.country || ''
        ...
        <div class="form-group" style="margin-bottom: 1rem;">
          ...
          <input type="text" class="form-input office-address" value="${
						office.address || ''
	...
	if (profile.benefits && profile.benefits.length > 0) {
        ...
        <div style="display: grid; grid-template-columns: 200px 1fr auto; gap: 1rem; align-items: end;">
          <div class="form-group">
            ...
            <select class="form-input benefit-type">
              ...
              <option value="Health Insurance" ${
								benefit.type === 'Health Insurance' ? 'selected' : ''
							}>Health Insurance</option>
              <option value="Flexible Hours" ${
								benefit.type === 'Flexible Hours' ? 'selected' : ''
							}>Flexible Hours</option>
              <option value="Remote Work" ${
								benefit.type === 'Remote Work' ? 'selected' : ''
							}>Remote Work</option>
              <option value="Learning Budget" ${
								benefit.type === 'Learning Budget' ? 'selected' : ''
							}>Learning Budget</option>
              <option value="Gym Membership" ${
								benefit.type === 'Gym Membership' ? 'selected' : ''
							}>Gym Membership</option>
              <option value="Free Lunch" ${
								benefit.type === 'Free Lunch' ? 'selected' : ''
							}>Free Lunch</option>
              <option value="Stock Options" ${
								benefit.type === 'Stock Options' ? 'selected' : ''
							}>Stock Options</option>
              <option value="Paid Time Off" ${
								benefit.type === 'Paid Time Off' ? 'selected' : ''
							}>Paid Time Off</option>
              <option value="Other" ${
								benefit.type === 'Other' ? 'selected' : ''
							}>Other</option>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-input benefit-description" value="${
							benefit.description || ''
	...
	if (profile.socialLinks && profile.socialLinks.length > 0) {
        ...
        <div style="display: grid; grid-template-columns: 200px 1fr auto; gap: 1rem; align-items: end;">
          <div class="form-group">
            ...
            <select class="form-input social-platform">
              ...
              <option value="Glassdoor" ${
								link.platform === 'Glassdoor' ? 'selected' : ''
							}>Glassdoor</option>
              <option value="Other" ${
          ...
          <div class="form-group">
            ...
            <input type="url" class="form-input social-url" value="${
							link.url || ''
						}" placeholder="https://..." />
          </div>
...
function saveCompanyProfile() {
	try {
		...
		if (!userId) {
			window.notify.warning('Please login first to save your company profile!');
			setTimeout(() => {
		...
		if (!companyName || !industry || !email) {
			window.notify.warning(
				'Please fill in required fields: Company Name, Industry, and Email'
			);
			return;
		}
		...
		window.dispatchEvent(
			new CustomEvent('companyProfileUpdated', { detail: profileData })
		);
		localStorage.setItem('companyProfileLastUpdated', Date.now().toString());

		window.notify.success('Company profile saved successfully!');

		console.log('Company profile saved with key:', profileKey);
		console.log('Saved Company Profile:', profileData);
	} catch (error) {
		console.error('Error saving company profile:', error);
		window.notify.error('Failed to save company profile. Please try again.');
	}
}
	...
	if (logoUpload) {
		logoUpload.addEventListener('change', function (e) {
			...
			if (!file.type.startsWith('image/')) {
				window.notify.warning('Please upload an image file');
				return;
			}
			...
			reader.onload = function (event) {
				...
				const logoAvatar = document.querySelector('.profile-avatar-large');
				if (logoAvatar) {
					logoAvatar.innerHTML = `<img src="${logoDataUrl}" alt="Company Logo" style="width: 100%; height: 100%; object-fit: cover;" />`;
				}
				...
				console.log('Company logo uploaded and saved');
			};
